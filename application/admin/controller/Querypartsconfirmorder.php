...
class Querypartsconfirmorder extends Controller
{
	/*查询订单渲染方法*/
    public function querypartsconfirmorder(){
        ...
        $editpartspower = ($role_info[0]['maintain_permission'])&0x02;
        $deletepartspower = ($role_info[0]['maintain_permission'])&0x04;

        $this->assign('editpartspower',$editpartspower);
        $this->assign('deletepartspower',$deletepartspower);
    ...
    /*导出配件确认单*/
    public function exportpartsconfirmorder(){
        $queryuserinfo = session("user_querypower");
        $organizename = "";
    ...
    /*打印配件确认单*/
    public function printpartsconfirmorder(){
        $template_name = "配件确认单.xlsx";
        $type=0x05;
        $cs_id = $_GET['cs_id'];
        $file_name = '配件确认单';
        $file_extend = 'xlsx';
